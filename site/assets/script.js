...
(function () {
  ...
  /* ===========================
     ...
     Robusto: fallback para browsers sem IO, visível
     inicial imediato, e timeout de segurança
     =========================== */
  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const fadeElements = document.querySelectorAll('.fade-in');

  // Função auxiliar para tornar visível
  const makeVisible = (el) => {
    el.classList.add('visible');
  };

  if (prefersReducedMotion || !('IntersectionObserver' in window)) {
    // Sem animação: revelar tudo imediatamente
    fadeElements.forEach(makeVisible);
  } else {
    // Hero elements e qualquer coisa já no viewport devem aparecer imediatamente
    ...
    // Revelar imediatamente o que já está visível
    revealInViewport();

    // Observer com threshold=0 e rootMargin generoso para disparar cedo
    ...
    // Fallback de segurança: após 500ms, qualquer elemento ainda invisível recebe visible
    setTimeout(() => {
      document.querySelectorAll('.fade-in:not(.visible)').forEach(makeVisible);
    }, 500);
  }
